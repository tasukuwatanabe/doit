todos = [
  ...
  { title: "飲み会", label: "娯楽", date: 3, status: true },
...
def createTodo(user, title, status=false, date, label)
...
users.each do |user|
  for i in 1..todos.length do
    todo = todos[i-1]
    createTodo(user, todo[:title], todo[:status], todo[:date], getLabelId(user, todo[:label]))
  end
end